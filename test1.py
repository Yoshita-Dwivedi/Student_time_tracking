...
# Track student presence without uniform detection
class StudentTracker:
    ...
    def update_presence(self, student_id, in_frame, current_time):
        ...
        if in_frame and not student["in_frame"]:
            ...
            if student["first_seen"] is None:
                ...
                student["time_in"] = datetime.fromtimestamp(current_time).strftime(
                    "%H:%M:%S"
                )
    ...
    def final_update(self, current_time):
        for student_id, data in self.students.items():
            ...
            if data["last_seen"]:
                data["last_seen"] = datetime.fromtimestamp(data["last_seen"]).strftime(
                    "%H:%M:%S"
                )
...
def main():
    ...
    st.set_page_config(
        page_title="Classroom Tracker", page_icon="👨‍🎓", layout="wide"
    )
        ...
        if st.session_state.is_running:
            ...
            if hasattr(st.session_state, "tracker") and st.session_state.tracker:
                for student_id, data in st.session_state.tracker.students.items():
                    # Check if 'total_time' is not None before dividing
                    total_time_min = (data["total_time"] / 60) if data["total_time"] is not None else 0
                    if data["in_frame"] and data["start_time"]:
                    ...
                    with st.expander(f"**{data['name']}** ({student_id})"):
                        st.caption(f"{status} | {total_time_min:.2f} minutes")
                        if data.get('time_in'):
